...
//    for (byte ch8tone = tone_start ; ch8tone < tone_end) {
      for (byte steps = 0 ; steps < STEP_QUANT ; steps++) {
        if (clip[7][ch8Clip][steps] > 0) {
          tft.fillCircle((steps * STEP_FRAME_W) + DOT_OFFSET_X, ((clip[7][ch8Clip][steps] - tone_start) * STEP_FRAME_H) + DOT_OFFSET_Y, DOT_RADIUS, trackColor[ch8COLOR]);
        }
      }
//    }
    ...
    if (ts.touched()) {
      ...
      if (gridTouchX >= SEQ_GRID_LEFT && gridTouchX <= SEQ_GRID_RIGHT && gridTouchY >= SEQ_GRID_TOP && gridTouchY <= SEQ_GRID_BOTTOM) {
        ...
        if (clip[7][ch8Clip][step_number] == 0) {
          clip[7][ch8Clip][step_number] = ch8tone;
//          clipOctaves[7][ch8Clip][step_number] = ch8Octaves;
        ...
        else if (clip[7][ch8Clip][step_number] > 0) {
          clip[7][ch8Clip][step_number] = 0;
