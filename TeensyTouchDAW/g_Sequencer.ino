...
void melodicTickSequencer() {
  ...
  switch (lastPotRow) {
    case 0:
      ...
      if (enc_moved[0]) {
        pixelTouchX = constrain((pixelTouchX + (encoded[0] * TICK_PIXEL_WIDTH * allTracks[desired_instrument]->stepLenght)), 0, 320);
        drawCursorPixel();
        showCoordinates();
    ...
    case 2:
      ...
      if (enc_moved[2]) {
        allTracks[desired_instrument]->setSeqVelocity(encoded[2]);
      }
      break;
  }
  ...
  if (ts.touched() || button[15]) {
    ...
    if (gridTouchX >= 18 && gridTouchY == 13) {
      for (int tickrs = 0; tickrs < TICKS_PER_BAR; tickrs++) {
        for (int polys = 0; polys < MAX_VOICES; polys++) {
          ...
          ctrack[desired_instrument].sequence[allTracks[desired_instrument]->clipToEdit].tick[tickrs].velo[polys] = 0;
        }
    ...
    if (pixelTouchX >= 28 && pixelTouchX <= 320 && gridTouchY >= SEQ_GRID_TOP && gridTouchY <= SEQ_GRID_BOTTOM) {
      if (!touched) {
        //some variables
        int desired_tick = (((pixelTouchX + TICK_PIXEL_WIDTH) - (2 * STEP_FRAME_W)) / TICK_PIXEL_WIDTH);
        int note = touched_note + allTracks[desired_instrument]->shownOctaves * 12;
        byte desired_velo = allTracks[desired_instrument]->seqVelocity;
          ...
          if (allTracks[desired_instrument]->voiceCount == polys) {
            if (ctrack[desired_instrument].sequence[allTracks[desired_instrument]->clipToEdit].tick[desired_tick].voice[polys] == VALUE_NOTEOFF) {
              for (int touched_ticks = 0; touched_ticks < allTracks[desired_instrument]->stepLenght; touched_ticks++) {
                ...
                ctrack[desired_instrument].sequence[allTracks[desired_instrument]->clipToEdit].tick[desired_tick + touched_ticks].velo[polys] = desired_velo;
