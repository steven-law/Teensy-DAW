  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (ts.touched()) {
      ...
      if ( PHRASE_SEGMENT_LENGTH == 16) {//only show clipselector when zoom is full in
        tft.drawRect(STEP_FRAME_W * 2, STEP_FRAME_H * 13, STEP_FRAME_W * 2, STEP_FRAME_H, trackColor[songModeSelectedTrack]); //draw a small rect to frame the clip info
        ...
        if (gridTouchX >= SEQ_GRID_LEFT && gridTouchX <= SEQ_GRID_RIGHT && constrainedtrackTouchY >= 0 && constrainedtrackTouchY <= 7) {
          songModeSelectedTrack = constrainedtrackTouchY;
          songModeSelectedClip = gridTouchX - 2;
          tft.setFont(Arial_8);
          tft.setTextColor(ILI9341_WHITE);
          ...
          tft.print("T");
          tft.print(songModeSelectedTrack + 1);
          tft.print("-");
          tft.print("C");
          tft.print(arrangment1[songModeSelectedTrack][songModeSelectedClip] + 1);
        }
        ...
        if (gridTouchX == 4 && gridTouchY == 13) {
          arrangment1[songModeSelectedTrack][songModeSelectedClip]++;
        }
