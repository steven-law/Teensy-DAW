...
void gridchannel7Sequencer () {   //static Display rendering
  clearWorkSpace();
  drawMelodicSequencerStatic(ch7COLOR);
  tft.print(ch7Octaves);
}

void channel7Sequencer () {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    for (byte tone = tone_start ; tone < tone_end ; tone++) {
      for (byte steps = 0 ; steps < STEP_QUANT ; steps++) {
        if (channel7Clip[ch7Clip][tone][steps]) {
          tft.fillCircle((steps * STEP_FRAME_W) + DOT_OFFSET_X, ((tone - tone_start) * STEP_FRAME_H) + DOT_OFFSET_Y, DOT_RADIUS, trackColor[ch7COLOR]);
        }
      }
    }
    ...
    if (ts.touched()) {
      ...
      if (gridTouchX >= SEQ_GRID_LEFT && gridTouchX <= SEQ_GRID_RIGHT && gridTouchY >= SEQ_GRID_TOP && gridTouchY <= SEQ_GRID_BOTTOM) {
        ...
        if (channel7Clip[ch7Clip][ch7tone][step_number] == LOW) {
          channel7Clip[ch7Clip][ch7tone][step_number] = HIGH;
          tft.fillCircle((gridTouchX - 2) * STEP_FRAME_W + DOT_OFFSET_X, (gridTouchY - 1) * STEP_FRAME_H + DOT_OFFSET_Y, DOT_RADIUS, trackColor[ch7COLOR]); //draw the active steps circles
        }
        else if (channel7Clip[ch7Clip][ch7tone][step_number] == HIGH) {
          channel7Clip[ch7Clip][ch7tone][step_number] = LOW;
